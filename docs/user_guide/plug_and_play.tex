...
% #                                                                                    #
% # This is part of the documentation of the HPC\_Drug software                         #
% #                                                                                    #
	...
	\subsection{HREM for FS-DAM Protein-Ligand binding free energy (main.py)}
		...
		\subsubsection{The input file}
			...
			\begin{itemize}
				...
				\item number\_of\_hrem\_replicas\_per\_battery\_bound = the number of HREM replicas per each Battery bound state, default 8

				\item number\_of\_hrem\_replicas\_per\_battery\_unbound = the number of HREM replicas per each Battery unbound state, default 8

				\item bound\_batteries = number of bound batteries per HREM, default auto
				
				\item unbound\_batteries = number of unbound batteries per HREM, default auto
				
				\item n\_steps\_bound = number of MD steps (reference state) for bound HREM, dafault auto
				\item n\_steps\_unbound = number of MD steps (reference state) for unbound HREM, dafault auto
				\item timestep\_bound = timestep for bound HREM, dafault auto
				\item timestep\_unbound = timestep for unbound HREM, dafault auto

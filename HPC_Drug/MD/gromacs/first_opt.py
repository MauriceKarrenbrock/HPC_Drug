...
class GromacsFirstOptimization(gromacs_input.GromacsInput):
    ...
    def __init__(self,
        ...
        self.command_string = [
            ...
            [f"{self.MD_program_path}", "grompp", "-f", f"{self.mdp_file}", "-c", f"{self.Protein.gro_file}", "-p", f"{self.Protein.top_file}", "-o", f"{self.output_tpr_file}", "-maxwarn", "1000"],
...
class GromacsFirstOptimizationOnlyLigand(gromacs_input.GromacsInput):
    ...
    def execute(self):
        ...
        for i in range(len(Ligand)):
            ...
            command_string = [
                [f"{self.MD_program_path}", "editconf", "-f", f"{Ligand[i].gro_file}", "-d", "5", "-bt", "triclinic", "-angles", "90", "90", "90", "-o", f"{Ligand[i].gro_file}"],
                [f"{self.MD_program_path}", "grompp", "-f", f"{self.mdp_file}", "-c", f"{Ligand[i].gro_file}", "-p", f"{Ligand[i].top_file}", "-o", f"{os.getcwd()}/{Ligand[i].resname}_only_ligand.tpr", "-maxwarn", "1000"],
